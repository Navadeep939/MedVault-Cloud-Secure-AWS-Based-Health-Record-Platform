...
graph TD
    user((User)) -->|HTTPS/TLS| waf[AWS WAF]
    waf -->|Filter| igw[Internet Gateway]
    
    subgraph VPC [AWS VPC]
        subgraph Public_Subnet [Public Subnet]
            ec2[EC2: Flask Web App]
        end
        
        subgraph Private_Subnet [Private Subnet]
            logic[AI Logic Module]
            rds[(RDS: PostgreSQL)]
        end
        
        igw --> ec2
        ec2 -->|Internal API| logic
        ec2 -->|SQL| rds
    end
    
    subgraph AWS_Services [Managed Services]
        s3[(S3 Bucket<br/>Encrypted Vault)]
        kms[AWS KMS]
        gemini[Gemini API]
    end
